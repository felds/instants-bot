/// <reference path="./src/types.d.ts" />

import "./src/dm";
import "./src/events";
import { logger } from "./src/logging";

logger.fatal("Lá vem o negão.");
console.log(`
  Lá vem o negão
  Cheio de paixão
  Te catá, te catá, te catá
  Querendo ganhar todas menininhas
  Nem coroa ele perdoa não
  Fungou no cangote
  Da linda morena
  Te catá, te catá, te catá
  Loirinha a cafungada do negão
  É um problema
  Loirinha a cafungada do negão
  É um problema
  Lá vem o negão
  Lá vem o negão
  Cheio de paixão
  Te catá, te catá, te catá
  Querendo ganhar todas menininhas
  Nem coroa ele perdoa não
  Fungou no cangote
  Da linda morena
  Te catá, te catá, te catá
  Loirinha a cafungada do negão
  É um problema
  Loirinha a cafungada do negão
  É um problema
  Dá licença
  Se ninguém soube lhe amar
  Pode se preparar chegou a salvação
  Só alegria, pode se arrumar
  Lá vem o negão
  Mas se é compromissada
  É melhor não vacilar
  Basta um sorriso no olhar
  Para o negão te catar
  Lá vem o negão
  Cheio de paixão
  Te catá, te catá, te catá
  Querendo ganhar todas menininhas
  E nem coroa ele perdoa não
  Fungou no cangote
  Da linda morena
  Te catá, te catá, te catá
  Loirinha a cafungada do negão
  É um problema
  Loirinha com a cafungada do negão
  É um problema
  Tem mulher solteira aí? Tem mulher solteira? Tem mulher solteira?
  Vem negão, vem depressa
  É o mulherio a gritar
  Vem negão que a hora é essa
  Vamos deitar e rolar
  Na praia, na rua, no supermercado
  Na feira é a maior curtição
  As danadinhas já vem requebrando
  Pra dançar com esse negão
  Lá vem o negão
  Cheio de paixão
  Te catá, te catá, te catá
  Querendo ganhar todas menininhas
  E nem coroa ele perdoa não
  Fungou no cangote
  Da linda morena
  Te catá, te catá, te catá
  Loirinha a cafungada do negão
  É um problema
  Do Brasil para o mundo
  Loirinha com a cafungada do negão
  É um problema
  Loirinha a cafungada do negão
  É um problema
  Loirinha a cafungada do negão
  É um problema
  Quem gostou da um gritão
  Ulisses Souza
  Lá vem o negão
  Sou eu
`);
logger.fatal("Cravo e Canella");
